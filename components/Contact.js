    ...
    return (
      <section id="contact" className="section-spacing relative bg-[#020817]" data-name="contact" data-file="components/Contact.js">
        ...
        <div className="container mx-auto max-w-6xl px-4 relative z-10">
          <div className="flex flex-col lg:flex-row gap-0 items-stretch border border-blue-900/40 rounded-3xl overflow-hidden shadow-2xl shadow-blue-950/20">

            <div className="flex-1 bg-slate-950 p-8 md:p-12 border-b lg:border-b-0 lg:border-r border-blue-900/30">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-900/30 border border-blue-800/50 mb-8">
                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">System Online</span>
              </div>

              <h2 className="text-4xl font-black text-white mb-6 uppercase tracking-tight leading-none">
                Establish <br />
              ...
              <div className="space-y-6">
                {contactInfo.map((info) =>
                  info.label === 'Email' ? (
                    ...
                    >
                      <div className="w-10 h-10 rounded bg-blue-950/50 flex items-center justify-center border border-blue-900/50 group-hover:border-blue-600 transition-all">
                        <div className={`${info.icon} text-lg ${info.color}`}></div>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase font-bold text-blue-700 tracking-tighter leading-none mb-1">{info.label}</p>
                        <p className="text-slate-300 font-mono text-sm group-hover:text-blue-400 transition-colors">{info.value}</p>
                      </div>
                    </a>
                  ) : (
                    <a key={info.label} href={info.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 group">
                      <div className="w-10 h-10 rounded bg-blue-950/50 flex items-center justify-center border border-blue-900/50 group-hover:border-blue-600 transition-all">
                        <div className={`${info.icon} text-lg ${info.color}`}></div>
                      </div>
                      <div>
              ...
              <div className="mt-16 pt-8 border-t border-blue-900/20">
                <div className="flex gap-1 mb-2">
                  {[...Array(12)].map((_, i) => (
                    <div key={i} className={`h-8 w-1.5 rounded-full ${i < 8 ? 'bg-blue-600/40' : 'bg-slate-800'}`}></div>
                  ))}
                </div>
              ...
              <form className="space-y-8 relative">
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="relative group">
                    <input
                      ...
                      className="peer w-full bg-transparent border-b-2 border-slate-800 py-3 text-white focus:outline-none focus:border-blue-800 transition-colors"
                      placeholder=" "
                    />
                    <label
                      htmlFor="name"
                      className={`absolute left-0 uppercase text-[10px] font-bold tracking-widest pointer-events-none transition-all bg-[#020617] px-2 py-0.5 rounded z-10 ${
                        formData.name
                          ? '-top-7 text-blue-600'
                ...
                <div className="relative group">
                  <textarea
                    ...
                    className="peer w-full bg-transparent border-b-2 border-slate-800 py-3 text-white focus:outline-none focus:border-blue-800 transition-colors resize-none"
                    placeholder=" "
                  ></textarea>
                  <label
                    htmlFor="msg"
                ...
                <div className="pt-6 relative">
                  <button
                    ...
                    className={`group w-full py-4 font-bold uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg shadow-blue-950/40 border ${
                      status === 'sending'
                        ? 'bg-slate-800 border-slate-700 text-slate-500 cursor-not-allowed'
                        : 'bg-[#0a192f] border-blue-900 text-blue-400 hover:bg-blue-950 hover:border-blue-600'
                    }`}
                  >
                    {status === 'sending' ? 'Transmitting...' : 'Initiate_Transmission'}
                    <div className={`text-base transition-transform ${status === 'sending' ? 'icon-loader animate-spin' : 'icon-arrow-right group-hover:translate-x-2'}`}></div>
                  </button>
